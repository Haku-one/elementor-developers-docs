 ...
 * Правильная реализация с использованием хуков WooCommerce
 */
...
class WC_Volume_Discounts {
    ...
    public function init() {
        ...
        add_action('woocommerce_product_after_variable_attributes', array($this, 'add_variation_volume_discount_fields'), 10, 3);
        add_action('woocommerce_save_product_variation', array($this, 'save_variation_volume_discount_fields'), 10, 2);
    ...
    /**
     * Получение настроек скидок для товара
     */
    public function get_discount_settings($product_id) {
        $discount_settings = array();
        
        // Для вариаций используем настройки родительского товара
        $parent_id = wp_get_post_parent_id($product_id);
        if ($parent_id) {
            $product_id = $parent_id;
        }
        
        // Получаем настройки из метаполей
        for ($i = 1; $i <= 7; $i++) {
    ...
    public function apply_volume_discounts($cart) {
        ...
        foreach ($cart->get_cart() as $cart_item_key => $cart_item) {
            ...
            $discount_percentage = $this->get_discount_percentage($quantity, $actual_id);
    ...
    public function display_discount_in_mini_cart($product_name, $cart_item, $cart_item_key) {
        if (isset($cart_item['volume_discount']) && $cart_item['volume_discount'] > 0) {
            if (strpos($product_name, 'volume-discount-info') === false) {
                $product_name .= '<div class="volume-discount-info">Скидка: ' . $cart_item['volume_discount'] . '%</div>';
            }
        }
        return $product_name;
    }
    ...
    public function ajax_get_discount_info() {
        ...
        $discount_percentage = $this->get_discount_percentage($quantity, $product_id);
        ...
        <div id="<?php echo esc_attr($unique_id); ?>" class="volume-discount-controls" data-product-id="<?php echo esc_attr($product_id); ?>">
            ...
            <?php if ($atts['show_discounts'] === 'yes'): ?>
                <div class="volume-discount-table">
                    ...
                    <ul>
                        <?php foreach ($this->get_discount_settings($product_id) as $setting): ?>
                            <li>
        ...
        jQuery(document).ready(function($) {
            ...
            function findVariation() {
                ...
                if (matchedVariation) {
                    ...
                    container.find('.single_variation').html('<div class="woocommerce-variation-price">' + matchedVariation.price_html + '</div>');
    ...
    /**
     * Добавление полей в админке товара
     */
    ...
    /**
     * Добавление полей для вариаций
     */
    public function add_variation_volume_discount_fields($loop, $variation_data, $variation) {
        echo '<div class="options_group">';
    ...
    /**
     * Сохранение полей вариации
     */
    public function save_variation_volume_discount_fields($variation_id, $loop) {
        for ($i = 1; $i <= 7; $i++) {
            $min_field = '_volume_discount_min_' . $i;
