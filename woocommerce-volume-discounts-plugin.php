...
Description: Полная система скидок по объему для WooCommerce с поддержкой простых и вариативных товаров. Включает шорткод с интегрированными стилями и JavaScript.
Version: 1.0.0
Author: Your Name
Author URI: https://example.com
Text Domain: wc-volume-discounts
...
// Проверка активации WooCommerce
add_action('plugins_loaded', 'wc_volume_discounts_check_woocommerce');
function wc_volume_discounts_check_woocommerce() {
    if (!class_exists('WooCommerce')) {
        add_action('admin_notices', 'wc_volume_discounts_woocommerce_missing_notice');
        return;
    }
    
    // Инициализация плагина
    wc_volume_discounts_init();
}

function wc_volume_discounts_woocommerce_missing_notice() {
    echo '<div class="notice notice-error"><p>' . __('WooCommerce Volume Discounts требует активации WooCommerce.', 'wc-volume-discounts') . '</p></div>';
}

function wc_volume_discounts_init() {
    ...
    function display_volume_discount_in_cart($cart_item_data, $cart_item) {
        if (isset($cart_item['volume_discount']) && $cart_item['volume_discount'] > 0) {
            $cart_item_data[] = array(
                'name' => __('Скидка', 'wc-volume-discounts'),
    ...
    function display_discount_in_mini_cart($product_name, $cart_item, $cart_item_key) {
        if (isset($cart_item['volume_discount']) && $cart_item['volume_discount'] > 0) {
            if (strpos($product_name, 'mini-cart-discount') === false) {
                $product_name .= '<div class="mini-cart-discount">' . __('Скидка:', 'wc-volume-discounts') . ' ' . $cart_item['volume_discount'] . '%</div>';
            }
        }
        return $product_name;
    }
    ...
    function replace_mini_cart_subtotal($fragments) {
        if (isset($fragments['div.elementor-menu-cart__subtotal'])) {
            ...
            $fragments['div.elementor-menu-cart__subtotal'] = '<div class="elementor-menu-cart__subtotal"><strong>' . __('Подытог:', 'wc-volume-discounts') . '</strong> <span class="woocommerce-Price-amount amount"><bdi>' . $formatted_subtotal . '</bdi></span></div>';
        }
        return $fragments;
    }
    ...
    function get_volume_discount_info_ajax() {
        ...
        if ($product_id <= 0) {
            wp_send_json_error(array('message' => __('Неверный ID товара', 'wc-volume-discounts')));
            return;
        }
        
        $product = wc_get_product($product_id);
        if (!$product) {
            wp_send_json_error(array('message' => __('Товар не найден', 'wc-volume-discounts')));
            return;
        }
        ...
        if ($discount_percentage > 0) {
            ...
            $html .= '<p>' . __('Скидка:', 'wc-volume-discounts') . ' <strong>' . $discount_percentage . '%</strong></p>';
            $html .= '<p>' . __('Цена со скидкой:', 'wc-volume-discounts') . ' <strong>' . number_format($discounted_price, 0, '.', '') . ' руб.</strong></p>';
            $html .= '<p>' . __('Экономия:', 'wc-volume-discounts') . ' <strong>' . number_format($savings * $quantity, 0, '.', '') . ' руб.</strong></p>';
            $html .= '</div>';
        }
    ...
    function add_to_cart_with_volume_discount_ajax() {
        ...
        if (!$cart_item_key) {
            wp_send_json_error(array('message' => __('Не удалось добавить товар в корзину', 'wc-volume-discounts')));
            return;
        }
        ...
        wp_send_json_success(array(
            'message' => __('Товар добавлен в корзину', 'wc-volume-discounts'),
            'fragments' => $fragments
        ));
    }
    ...
    function volume_discount_controls_shortcode($atts) {
        if (!function_exists('wc_get_product')) return '<p>' . __('WooCommerce не активен.', 'wc-volume-discounts') . '</p>';
        
        $atts = shortcode_atts(array(
            'product_id' => 0,
            'button_text' => __('Добавить в корзину', 'wc-volume-discounts'),
        ...
        if (empty($atts['product_id']) || $atts['product_id'] == 0) {
            global $product;
            if (!$product) return '<p>' . __('Товар не найден.', 'wc-volume-discounts') . '</p>';
            $product_id = $product->get_id();
            $product = wc_get_product($product_id);
        } else {
            $product_id = intval($atts['product_id']);
            $product = wc_get_product($product_id);
            if (!$product) return '<p>' . sprintf(__('Товар с ID %s не найден.', 'wc-volume-discounts'), $product_id) . '</p>';
        }
        ...
        <div id="<?php echo esc_attr($unique_id); ?>" class="volume-discount-controls" data-product-id="<?php echo esc_attr($product_id); ?>">
            ...
            <form class="variations_form cart" method="post" enctype="multipart/form-data" data-product_id="<?php echo esc_attr($product_id); ?>">
                ...
                <div class="variations">
                    ...
                    <div class="reset-variations-wrapper">
                        <a class="reset_variations" href="#" style="visibility: hidden;"><?php _e('Очистить', 'wc-volume-discounts'); ?></a>
                    </div>
                </div>
            ...
            <div class="volume-discount-table">
                <p><?php _e('Скидки по количеству:', 'wc-volume-discounts'); ?></p>
                <ul>
                    <?php foreach ($discount_settings as $setting): ?>
                        <li>
                            <?php if ($setting['max'] < 999999): ?>
                                <?php printf(__('От %s до %s шт: %s%%', 'wc-volume-discounts'), esc_html($setting['min']), esc_html($setting['max']), esc_html($setting['percentage'])); ?>
                            <?php else: ?>
                                <?php printf(__('От %s шт: %s%%', 'wc-volume-discounts'), esc_html($setting['min']), esc_html($setting['percentage'])); ?>
                            <?php endif; ?>
                        </li>
                    <?php endforeach; ?>
                </ul>
            ...
            if (isVariable) {
                ...
                $form.on("change", "select", function() {
                    ...
                    if (matchingVariation) {
                        ...
                        $form.find(".single_variation").html("<div class=\"woocommerce-variation-unavailable\"><?php _e('Выберите опцию', 'wc-volume-discounts'); ?></div>");
                ...
                $form.on("submit", function(e) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                $container.find(".cart-notification").html("<div class=\"success-message\">" + response.data.message + " <a href=\"<?php echo wc_get_cart_url(); ?>\"><?php _e('Перейти в корзину', 'wc-volume-discounts'); ?></a></div>");
    ...
    function add_volume_discount_fields() {
        ...
        echo '<h3 style="padding-left:12px;">' . __('Настройки скидок по объему', 'wc-volume-discounts') . '</h3>';
        echo '<p style="padding-left:12px;">' . __('Укажите диапазоны количества товаров и соответствующие проценты скидок (до 7 правил)', 'wc-volume-discounts') . '</p>';
        ...
        for ($i = 1; $i <= 7; $i++) {
            ...
            woocommerce_wp_text_input(array(
                ...
                'label' => __('От (шт)', 'wc-volume-discounts'),
                'type' => 'number',
            ...
            if ($i < 7) {
                woocommerce_wp_text_input(array(
                    ...
                    'label' => __('До (шт)', 'wc-volume-discounts'),
                    'type' => 'number',
                    'custom_attributes' => array('step' => '1', 'min' => '1'),
                    'wrapper_class' => 'form-field'
                ));
            ...
            woocommerce_wp_text_input(array(
                ...
                'label' => __('Скидка (%)', 'wc-volume-discounts'),
                'type' => 'text',
                'wrapper_class' => 'form-field'
            ));
    ...
    function add_variation_volume_discount_fields($loop, $variation_data, $variation) {
        echo '<div class="options_group">';
        echo '<h4>' . __('Настройки скидок по объему для вариации', 'wc-volume-discounts') . '</h4>';
